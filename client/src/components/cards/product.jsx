function Product({ id, image, title, description, state, price }) {
  return (
    <a href={`/shop/${id}`}>
      <article className="shadow-[0px_0px_10px_rgba(0,0,0,0.10)] rounded-lg overflow-hidden">
        <picture className="">
          <img src={image} className="object-cover object-center h-56 w-full" />
        </picture>
        <div className="py-5 px-5">
          <h4 className="font-semibold text-gray-600 text-lg mb-2">{title}</h4>
