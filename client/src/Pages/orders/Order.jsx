...
function OrderPage() {
  ...
  function formatDateTime(dateTimeString) {
    ...
    return `${dateTime.toLocaleDateString(
      ...
      .toLocaleTimeString("es-CR", timeOptions)
      .replace("00:", "12: ")}`;
  }
  ...
  return (
    <main className="w-full flex flex-col mt-16 py-20 px-5 items-center">
      {data.length === 0 ? (
        <h1>Cargando...</h1>
      ) : (
        <section className="w-full max-w-5xl min-h-screen flex flex-col items-center">
          <div className="w-full">
            <button
              onClick={() => navigate(-1)}
              className="font-medium text-gray-600 flex gap-2 items-center"
            >
            ...
            <p className="text-2x1 font-medium text-gray-600">
              ...
              <b>
                {" "}
                {data.client.name} {data.client.lastName1}{" "}
                {data.client.lastName2}{" "}
              </b>
            </p>
            {admin ? (
              <></>
          ...
          <div className="w-full flex flex-col mt-10">
            <h2 className="text-2xl font-medium text-gray-600">Ubicación</h2>
            <ul className="text-2x1 font-medium text-gray-600 list-disc list-inside ml-9 my-5">
              <li>
                <b>Provincia:</b> {data.address.province}
              </li>
              <li>
                <b>Cantón:</b> {data.address.canton}
              </li>
              <li>
                <b>Distrito:</b> {data.address.district}
              </li>
              <li>
                <b>Dirección exacta:</b> {data.address.specificAddress}
              </li>
            </ul>
            <h2 className="text-2xl font-medium text-gray-600">Productos</h2>
            <table className="w-full text-base text-left text-gray-500 mt-5 border-gray-200 border-2 table-fixed">
              ...
              <tbody>
                {data.products.map((productObj) => {
                  return (
                    <tr
                      className="bg-white border-b [&>td]:px-4 [&>td]:py-4"
                    ...
                    >
                      ...
                      <td className="border-l-2 border-gray-200">
                        {formatCurrency(productObj.product.price)}
                      </td>
                      <td className="border-l-2 border-gray-200">
                        {formatCurrency(
                          productObj.product.price * productObj.amount
                        )}
                      </td>
                    </tr>
                  );
          ...
          {admin ? (
            <div className="mt-10 w-72">
              ...
              <button
                className="mt-3 bg-indigo-500 hover:bg-indigo-400 transition-colors py-1 font-medium text-white w-full text-lg rounded-md"
